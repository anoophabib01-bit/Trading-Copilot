...
// This process runs the trader's entire co-pilot: WS server, engulf/FVG/SFP
...
// existed. This is the fail-open behavior the trader asked for after seeing
...
// 2026-07-25 (the trader: "D:\co-pilot DATA — save data of all the account here…
...
//     trade_journal.json            ← LIFETIME journal (about the trader, not an account)
//
...
// untouched at the in-project fallback path. the trader only caught it because he
...
// the trader asked whether a better/paid model would improve the agents. The honest
...
// the trader: "can i add screenshot of the data which i trade and be saved in same
...
//
// FIX (2026-07-21, "DOMMatrix is not defined" on the trader's real machine): pdf-parse
...
// from this session's Linux sandbox (a live-mounted shared folder, not the trader's
...
// Windows one). On the trader's Windows machine that native module fails to load,
...
const JESSI_PERSONA = `You are Jessi Livermore — the trader's accountability coach and psychological companion for prop-firm trading, built into his MNQ Co-Pilot app. Named after the trader Jesse Livermore (the trader's own spelling, not corrected).

Who you're talking to: the trader, IST timezone (IST). Trades MNQ (Micro Nasdaq) and MGC (Micro Gold) as a prop-firm scalper. Lifetime losses to recover: ~$10,784.50 across blown accounts and eval fees. He has blown 16 prop accounts before this one — every single one hit its Max Loss Limit, and every post-mortem shows the same handful of failure modes (trade-count escalation, revenge clusters, inverted R:R, holding losers, multi-instrument days, giving back gains after being up). You know this history. Reference it plainly when it's relevant — don't soften it.
...
## CURRENT MISSION (the trader's own framing, 2026-07-25)
...
## VARIETY — DO NOT SOUND LIKE A SCRIPT (the trader's explicit complaint)
...
## WHAT YOU CAN DO ON THE CHART (updated 2026-07-22 — the trader explicitly asked for this after being warned it's untested)
...
the trader named JadeCap ("Trading Isn't Hard, It's Misunderstood") as mentor-level and wants these enforced, not just referenced:
...
const JESSI_TV_TOOLS = [
  ...
  { type: 'function', function: { name: 'draw_shape', description: 'Draw on the chart: horizontal_ray (PREFERRED for marking a high/low level — anchors at point.time, the actual candle where that high/low occurred, extends rightward only, matching how the trader marks levels himself), horizontal_line (rarely wanted — spans the ENTIRE chart both directions regardless of point.time, only use if he explicitly asks for a full-chart line), trend_line, rectangle, or text.', parameters: { type: 'object', properties: { shape: { type: 'string', enum: ['horizontal_ray', 'horizontal_line', 'trend_line', 'rectangle', 'text'] }, point: { type: 'object' }, point2: { type: 'object' }, text: { type: 'string' }, color: { type: 'string' } }, required: ['shape', 'point'] } } },
...
const JESSI_APP_TOOLS = [
  ...
  // 2026-07-27: the trader's 5-book trading library (Stock Market Wizards, Trading
  ...
  { type: 'function', function: {
    name: 'search_books',
    description: 'Search the trader\'s trading book library for passages relevant to a topic (e.g. "revenge trading", "position sizing", "probabilistic thinking", "cutting losers"). Returns the most relevant passages with book title + a rough location, so you can quote or paraphrase them when coaching the trader. Use this when he asks what a book says about something, or when grounding advice in a specific author\'s framework would help more than generic coaching.',
...
const JESSI_PERSONA_VOICE = `You are Jessi Livermore, the trader's trading accountability coach (voice mode) in his MNQ Co-Pilot app. the trader trades MNQ/MGC on a prop-firm prop account. He has blown 16 prop accounts, all by hitting the max-loss limit, always via the same failure modes: trade-count escalation, revenge re-entries, oversized "high-conviction" trades, holding losers, trading multiple instruments, and giving back gains after being green. You know this and name it plainly — coach, not cheerleader. Grade PROCESS over P&L: a green day with broken rules is a failure; a red day with clean rules is a win. Never validate a revenge trade, an oversize, or an "I'll get it back."
Use app_get_data(section) for anything about the open account (status/cost/insights/trades/scalp/checklist/roadmap) instead of guessing — "scalp" gives the per-day hold-time/gap breakdown, use it whenever the trader asks how his scalping looked on a given day. Use app_do to act in the app. You can NEVER place trades. For the two destructive actions (switch_account, clear_insights) get a clear spoken confirmation first, then call again with confirm:true.
...
// 2026-07-27 — "numbers are the major game changer" (the trader, verbatim, after
// the bucket-key bug above shipped a wrong balance to chat). What the trader asked
...
function jessiAppGetData(section) {
  ...
  if (want('scalp')) {
    // Per-day scalping breakdown — added 2026-08-01 (the trader): he wants hold
...
function makeJessiToolExecutor(ws) {
  ...
  return async (name, args) => {
    if (JESSI_TV_TOOL_NAMES.has(name)) {
      ...
      // RESULT, not a thrown error, so the model reads it, tells the trader, and
      // carries on with the rest of the conversation.
      if (!mcpBridge.ready || !mcpBridge.tvConnected) {
        return `TradingView is disconnected — "${name}" is unavailable right now. Do not retry chart tools this turn. Tell the trader TradingView Desktop needs to be running and reconnected (Refresh in the app), and answer whatever you can from his app data instead.`;
      }
    ...
    if (name === 'app_do') {
      ...
      if (DESTRUCTIVE.has(action) && !(args && args.confirm === true)) {
        return `CONFIRMATION REQUIRED — "${action}" is destructive. Tell the trader out loud exactly what it will do, get a clear spoken "yes/confirm", then call app_do again with confirm:true. Do NOT run it yet.`;
      }
...
// FIX (2026-08-06, the trader: "power of 3 is always unclear" investigation
...
// 2026-08-06: "Where your head's at" (Alignment tab) — the trader's own dated
...
Do NOT call any chart tool (chart_get_state, quote_get, market_key_levels, data_get_ohlcv, data_get_pine_labels, draw_shape, draw_list, draw_remove_one, alert_*) — they will all fail until it reconnects. If the trader asks about the live chart, price or levels, tell him straight that TradingView is disconnected and he needs TradingView Desktop running, then Refresh in the app. Everything else about you works fine without it: his trade history, checklist, journal, breach record, patterns and the coaching all come from app data, not the chart.`);
...
async function maybeFetchTVSnapshot(text) {
  ...
  if (!mcpBridge.ready || !mcpBridge.tvConnected) return '(the trader asked about the chart, but TradingView MCP is not connected right now — tell him to check the connection.)';
  try {
    const [state, quote, levels] = await Promise.all([
      mcpBridge.callTool('chart_get_state', {}).catch(() => null),
      mcpBridge.callTool('quote_get', {}).catch(() => null),
...
const ANALYSIS_DEBATE_PERSONA = `You are the Technical Analysis Agent in the trader's MNQ Co-Pilot trading app. Your role in this debate is to argue PURELY from the market/technical data perspective.
...
the trader's entry framework requires ALL of these in sequence: (1) Daily bias clear, (2) 1H aligns with Daily, (3) Price at a pre-marked 4H zone, (4) 15M/5M reaction at zone, (5) 3M/1M trigger. Score the current setup against each step.`;

const JUDGE_PERSONA = `You are the Expert Judge in the trader's MNQ Co-Pilot trading app. You receive THREE arguments — from Jessi (discipline/psychology), the Technical Analysis agent (structure/levels), and the ICT Power of 3 agent (AMD phase: Accumulation / Manipulation / Distribution) — and you synthesize them into a single, definitive answer.
...
Context: the trader has blown 16 prop accounts. Every single one hit its Max Loss Limit through the same failure modes. The margin for error is zero. When in doubt, the answer is NO.`;
...
async function gatherAnalysisContext() {
  ...
  // Scalper/Claude/Post-Session (via buildJessiContext). the trader asked for it
...
async function handleDebateChat(ws, msg) {
  ...
  // try/catch. the trader hit a hard stuck chat ("the chat is crashed" — red stop
  ...
  try {
    ...
    // (the trader: "i want its active participation" — a side button wasn't the
...
// 2026-07-28, from the trader's own "ICT Power of 3 – The Ultimate Guide" PDF.
...
// heavily (the trader's explicit instruction), because that's where the phase
...
const ICT_PO3_PERSONA = `You are the ICT POWER OF 3 agent in the trader's MNQ/MGC trading co-pilot.
...
DAILY IS DELIBERATELY NOT IN YOUR DATA (2026-07-29, the trader's decision — he reads
...
## TIMEFRAME WEIGHTING (the trader's instruction) — two DIFFERENT jobs, don't blur them
...
**INSTRUMENT:** the symbol from the chart state (the trader trades MNQ1! and MGC1!,
...
## RULES
- ANY INSTRUMENT. the trader's main two are MNQ1! (Micro Nasdaq) and MGC1! (Micro
...
- You describe market STATE. You do NOT tell the trader to enter, size, or exit — his
  playbook rules and risk limits govern that, and other agents handle it.
- the trader has blown 16 accounts; the most valuable thing you can say is often
...
// 2026-07-29 (the trader): "for directional bias change from 60 bars to 30 bars and
...
// silently re-graded the Analysis panel too, which the trader did not ask for. So
...
// 2026-07-29 (the trader): "i want it to trigger me and give my an output in lower
...
//
// Phase logic, from the trader's ICT Power of 3 PDF:
...
// the trader's spec 2026-07-29: poll every 60s, alert on any phase change, chat
...
// Whatever symbol is currently on the trader's chart. The whole read path is
...
async function checkPo3Phase() {
  ...
  try {
    // SYMBOL-CHANGE RESET (2026-07-29). the trader trades MNQ1! and MGC1! and wants
...
// drives the trader's live chart and must not leave it on the wrong TF mid-session.
async function gatherPO3Context() {
  ...
  // 2026-07-29 (the trader): 4H dropped, 1H is now the higher-timeframe anchor for
  ...
  // FIX (2026-08-06, the trader: "power of 3 is always unclear"): this called
  ...
  // time — exactly what the trader reported ("power of 3 always unclear").
  ...
  try {
    ...
    parts.push('NOTE: Daily is deliberately NOT provided — the trader reads the daily candle himself. Do not claim a daily read.');
  } catch (e) {
...
// 2026-07-28: the trader wants a speaker button on chat messages so he can replay
...
async function handleTtsSpeak(ws, msg) {
  ...
  // THREE-TIER TTS (2026-07-28). the trader hit a hard 403 mid-session because
  ...
  // EDGE CIRCUIT-BREAKER (2026-07-28). the trader's diagnostic came back
...
const POST_SESSION_ANALYST_PERSONA = `You are the POST-SESSION ANALYST for the trader's MNQ/MGC prop trading co-pilot app.

## YOUR ROLE
You fire AFTER a trading session is over and the trader has uploaded his Performance CSV. Your job is a forensic, numbers-first breakdown of the session. You are NOT a coach or cheerleader — you are an auditor who also gives forward-looking guidance.
...
2. **PLAN ADHERENCE** — Did the trader write a pre-committed A+ cap before the session? Did he stay inside it? Grade: PASS or FAIL. (Rule #13 & #14: the plan-adherence grade overrides P&L — a green day that broke the cap is a FAIL.)
...
6. **PATTERN CHECK** — Compare today against the trader's 6 documented failure modes (trade count escalation, revenge clusters, inverted R:R, holding losers, multi-instrument, accounts up-then-crashed). Flag ANY match.
...
// ═══════════════════════════════════════════════════════════════════════════════
// the trader's brief: an expert who "understands human greed of trading" but ALSO
...
const SCALPER_PERSONA = `You are THE SCALPER — the trader's specialist scalping coach inside his MNQ Co-Pilot app. You are not Jessi (the general accountability coach) and not the Post-Session Analyst (the forensic auditor). You are the one who understands the CRAFT of scalping AND the greed that destroys it, and your entire value is in spotting the gap between the two in the trader specifically.

## WHO ANOOP IS (do not forget any of this)
- Trades MNQ/MGC micros on a prop-firm prop account (IST). Currently a $50K EVALUATION, slot s1.
...
- scalp_note_add — record a durable note about a specific day or trade (behaviour observed, pattern, agreed fix). Use this whenever the trader reviews session video with you, so the observation survives the chat.
...
const SCALPER_TOOLS = [
  ...
  { type: 'function', function: {
    name: 'scalp_note_add',
    description: 'Record a durable scalping note for a specific date, optionally tied to a specific trade number from the Journal table. Use this whenever you and the trader review session video or discuss a specific trade, so the observation persists beyond this chat and can be cited on later days. Kinds: "behaviour" (what he did and the emotional driver), "pattern" (a repeating tendency across days), "fix" (the concrete agreed change), "level" (analysis-quality observation about the level/zone he chose).',
  ...
  { type: 'function', function: {
    name: 'search_books',
    description: 'Search the trader\'s trading book library (Stock Market Wizards, Trading in the Zone, Intraday Trading Techniques, Prop Trading Secrets, TradeApp\'s Guide to Proprietary Trading) for relevant passages when grounding a coaching point in a specific author helps more than your own framing.',
...
async function handleScalperChat(ws, msg) {
  ...
  try {
    ...
    const seeded = [{ role: 'user', content: 'CONTEXT (auto-attached, not typed by the trader):\n' + seed.join('\n') }]
...
async function handleJessiVoiceSend(ws, msg) {
  ...
  try {
    ...
    if (typeof msg.transcript === 'string' && msg.transcript.trim()) {
      ...
      // (the trader's ask — "omnirouter as primary, groq as second option"),
    ...
    // the trader's ask for a human-sounding voice he can use all day. Ladder:
...
// patterns are about the trader, not about which prop account is currently active,
...
async function _getFullBarsUnlocked(tfCode, count) {
  ...
  try {
    ...
    // live: the trader's chart showed PDH/PDL as a ~$48 spread instead of the
...
// Changed from 15M to 30M on 2026-07-15 per the trader: the 15M version was firing
...
// ── Trend STRENGTH classifier (added 2026-07-28, the trader) ─────────────────────
// the trader asked for all three ways of reading strength combined, not any one
// alone:
...
function classifyTrendStrength(bars) {
  ...
  // UPGRADED 2026-07-28 (the trader): was computed off market_multi_tf's fixed
  ...
  // is a guess, not derived from the trader's data. Watch it against your own read
...
// work by switching the trader's LIVE chart timeframe, reading, then switching
...
// Updated 2026-07-22 per the trader: added previous week H/L alongside the
...
// spans the ENTIRE chart both directions, which is not how the trader marks
...
// Updated 2026-07-22 per the trader: NY levels now mark current week H/L + current
...
// chart the trader is already watching — no separate lookup needed. On-demand only
...
// FIX (2026-07-18, the trader's ask): calendar re-FETCHES now happen at exactly two
...
async function checkSessionPrep() {
  ...
  for (const [key, win] of Object.entries(SESSION_WINDOWS)) {
    ...
    if (mins >= prepStart && mins < win.startMin && sessionPrepFired[key] !== dateStr) {
      ...
      // CHANGED 2026-07-28 (the trader): stop auto-marking chart levels on this
...
// SINGLE-INSTANCE GUARD (2026-07-29, the trader: "if i open two windows are opening
