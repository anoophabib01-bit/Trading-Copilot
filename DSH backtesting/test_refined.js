...
function fullStats(t){ if(!t.length) return null; const net=t.reduce((a,b)=>a+b.net,0); const w=t.filter(x=>x.net>0),l=t.filter(x=>x.net<=0); const gw=w.reduce((a,b)=>a+b.net,0),gl=-l.reduce((a,b)=>a+b.net,0); let eq=0,peak=0,dd=0; for(const x of t){eq+=x.net;peak=Math.max(peak,eq);dd=Math.max(dd,peak-eq);} const byDay={}; for(const x of t){const d=new Date(x.time*1000).toISOString().slice(0,10); byDay[d]=(byDay[d]||0)+x.net;} const days=Object.values(byDay); const best=Math.max(...days); return {n:t.length,net,win:w.length/t.length*100,pf:gl>0?gw/gl:Infinity,dd,bestDay:best,consistency:net>0?best/net*100:null,days:days.length}; }
...
for (const adxMin of [30, 35]){
  let all=[];
  console.log('=== LONG-only breakout, ADX>='+adxMin+', +DI>-DI, daily $1000 cap ===');
  for (const [name,f] of Object.entries(files)){
    let t = runLongAdx(load(f),10,adxMin); t = applyCap(t,1000);
    const s=fullStats(t);
    if(!s){ console.log('  '+name+' (no trades)'); continue; }
    console.log('  '+name.padEnd(20)+'n='+String(s.n).padEnd(4)+' win '+(s.win.toFixed(0)+'%').padEnd(5)+' $'+s.net.toFixed(0).padEnd(8)+' PF '+s.pf.toFixed(2).padEnd(6)+' DD $'+s.dd.toFixed(0).padEnd(7)+' bestDay $'+s.bestDay.toFixed(0));
    all=all.concat(t);
  }
  all.sort((a,b)=>a.time-b.time); const agg=fullStats(all);
  console.log('  AGG: n='+agg.n+' net $'+agg.net.toFixed(0)+' PF '+agg.pf.toFixed(2)+' maxDD $'+agg.dd.toFixed(0)+' bestDay $'+agg.bestDay.toFixed(0)+' consistency '+(agg.consistency!=null?agg.consistency.toFixed(0)+'%':'n/a'));
  console.log('');
}
