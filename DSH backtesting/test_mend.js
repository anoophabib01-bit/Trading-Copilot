...
function runLongAdx(bars, lookback, adxMin){
  ...
  for(let i=warmup;i<bars.length;i++){
    ...
    if(!sim) continue;
    trades.push({time:bar.time, points:sim.points});
  }
  return trades;
}
...
const net2 = t => t.points*pv*2 - 0.95*2*2;
for(const t of all) t.net=net2(t);

// daily cap variants
function applyDailyCap(trades, cap){ const byDay={}; const kept=[]; for(const t of trades){ const d=new Date(t.time*1000).toISOString().slice(0,10); const p=byDay[d]||0; if(p>=cap) continue; kept.push(t); byDay[d]=p+t.net; } return kept; }
function stats(t){ if(!t.length) return null; const net=t.reduce((a,b)=>a+b.net,0); const w=t.filter(x=>x.net>0),l=t.filter(x=>x.net<=0); const gw=w.reduce((a,b)=>a+b.net,0),gl=-l.reduce((a,b)=>a+b.net,0); let eq=0,peak=0,dd=0; for(const x of t){eq+=x.net;peak=Math.max(peak,eq);dd=Math.max(dd,peak-eq);} const byDay={}; for(const x of t){const d=new Date(x.time*1000).toISOString().slice(0,10); byDay[d]=(byDay[d]||0)+x.net;} const days=Object.values(byDay); const best=Math.max(...days); return {n:t.length,net,pf:gl>0?gw/gl:Infinity,dd,bestDay:best,consist:net>0?best/net*100:null}; }
const months = (all[all.length-1].time-all[0].time)/86400/30.4;

console.log('=== 2 CONTRACTS - consistency fix comparison ===');
for (const [label, cap] of [['no cap (aggressive)', Infinity], ['$1200/day cap', 1200], ['$1000/day cap', 1000], ['$800/day cap', 800]]){
  const t = cap===Infinity ? all : applyDailyCap(all, cap);
  const s = stats(t);
  console.log('  '+label.padEnd(20)+'n='+String(s.n).padEnd(4)+'net $'+s.net.toFixed(0).padEnd(7)+'PF '+s.pf.toFixed(2).padEnd(6)+'DD $'+s.dd.toFixed(0).padEnd(6)+'bestDay $'+s.bestDay.toFixed(0).padEnd(8)+'consist '+(s.consist!=null?s.consist.toFixed(0)+'%':'n/a')+'  -> $'+(s.net/months).toFixed(0)+'/month');
}
console.log('');
console.log('months of data: '+months.toFixed(1));
console.log('to clear $3000 legally (consist <=40%): need bestDay <= $1200');
